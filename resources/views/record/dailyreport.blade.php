                                    ...
                                    <div class="card-header py-3 card-header-color">
                                        <div class="float-left">
                                            <h1 class="text-2xl leading-6 font-medium text-gray-900 text-left">
                                                မြန်မာ့အလင်း သတင်းစာတိုက်နေ့စဉ်ကြော်ငြာစာရင်း
                                            </h1>
                                        </div>
                ...
                @foreach ($sales as $sale)
                    <form action="" method="GET">
                        ...
                        <tr>
                            <td>{{ $i++ }}</td>
                            <td>{{ $sale->itemsku->name }}</td>
                            <td>{{ $sale->itemsku->category->name }} (Kyats {{ number_format("$sale->price", 2) }})
                            </td>
                            <td id="col1" style="text-align: right">
                                {{ number_format("$sale->discount_amount", 2) }} </td>
                            <td id="col2" style="text-align: right">{{ $sale->quantity }}</td>
                            <td id="col3" style="text-align: right">{{ number_format("$sale->total_amount", 2) }}
                            </td>
                            @if ($sale->cancel == '1')
                                <td><span class="bg-warning rounded p-1">Cancelled</span></td>
                            @else
