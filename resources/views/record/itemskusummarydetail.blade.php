...
<body>
    ...
    <form method="POST">
        ...
        <h3 class="text-2xl leading-6 font-medium text-gray-900 text-left">
            @if (!empty($start_date))
           {{ $itemskuname->name }} Sales between "{{$start_date}}" 1:00AM and "{{$end_date}}" 1:00PM.
            @else
        ...
        <table class="table table-responsive-sm table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- @dd( $sale->item); --}}
                    <td id="col1">{{ $sale->discount_amount}} </td>
                    <td id="col2">{{ $sale->total_inches }}</td>
                    <td id="col3"> {{ $sale->total_amount }}</td>
                    @if ($sale->cancel == 1)
                    <td>
                ...
                <tr>
                    <td colspan="3" align="right"><b>Total</b></td>
                    <td><i>{{number_format("$totalDiscountSum", 2)}}</i></td>
                    <td><i>{{$totalInchSum}}</i> </td>
